...
static inline byte Mul(byte a, byte b) {
	return !a || !b ? 0 : Powers[(Logs[a] + Logs[b]) % 255];
}
...
uint32_t ExpandedKey::Next() {
	...
	if (0 == m_i)
		...
		pt[0] = SubByte[pa[0]]; pt[1] = SubByte[pa[1]]; pt[2] = SubByte[pa[2]]; pt[3] = SubByte[pa[3]];
	}
...
void Aes::EncryptBlock(const ConstBuf& ekey, byte *data) {
	EncDec(ekey, (uint32_t*)data, SubByte, MixColumn);
}
...
void Aes::DecryptBlock(const ConstBuf& ekey, byte *data) {
	...
	std::reverse(b, e);
	EncDec(ekey, b, InvSubByte, InvMixColumn);
	std::reverse(b, e);
}
