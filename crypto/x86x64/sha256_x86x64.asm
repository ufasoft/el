...
IF X64
	Sha256Update_x86x64	PROC PUBLIC USES ZBX ZSI ZDI
	lea	r11, g_sha256_k
IFDEF __JWASM__
	mov	zdx, zdi	; UNIX: just for generating frame   rdi=state, rsi=data
ELSE
	mov	zsi, zdx
	mov	zdx, zcx	; Win64:  rcx=state, rdx=data
ENDIF
ELSE				; X64
	Sha256Update_x86x64	PROC PUBLIC USES ZBX ZSI ZDI, state:DWORD, data:DWORD
	mov	zsi, data
	mov	zdx, state
ENDIF				; X64
	push	zbp
...
ENDIF
	...
	and	ebx, eax
	xor	ebx, edi			; Ch(e, f, g) == g ^ (e &  (f ^ g))
	mov	edi, ebp
	add	edx, ebx			; w_i + h + Ch(e, f, g)

	ror	edi, 9				; (a>>>2) ^ (a>>>13) ^ (a>>>22)
	...
	mov	edi, ebp
	xor	ebx, [Base + 4]			; a^b
	xor	edi, [Base + 2*4]		; a^c
	and	ebx, edi
	xor	ebx, ebp			; Ma(a, b, c)
...
LOOP_ADD:
	...
	add	zdi, 4
	loop	LOOP_ADD
	add	zsp, 16*4 + (8+64)*4 + ZWORD_SIZE
	pop	zbp
	ret
