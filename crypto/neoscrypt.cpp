...
#define UCFG_NEOSCRYPT_MY 1
//#define UCFG_NEOSCRYPT_MY 0
...
void neoscrypt(const uchar *password, uchar *output, uint profile) {
	...
	AlignedMem am((N + 3) * r * 2 * SCRYPT_BLOCK_SIZE, 128);
...
#if UCFG_NEOSCRYPT_MY
	SalsaBlockPtr z = (SalsaBlockPtr)Z,
		x = (SalsaBlockPtr)X,
		tmp = (SalsaBlockPtr)Y;

	NeoScryptCore(x, tmp, V, r, rounds, N, dblmix);
...
CArray8UInt32 CalcNeoSCryptHash(const ConstBuf& password, int profile) {
	...
	case 3:
		return CalcSCryptHash(password);
	default:
		Throw(E_NOTIMPL);
	}
}
